...
export default async function ApplyPage({ params }: { params: Promise<{ roleId: string }> }) {
  ...
  return <main className="container page"><section className="card"><p className="eyebrow">McLink Careers</p><h1>{role.jobTitle}</h1><p>{role.department} ({role.roleCountry})</p><p>{role.jobDescription}</p></section>{isLiveAvatarConfigured() && <LiveAvatarInterview roleId={role.roleId} roleTitle={role.jobTitle} />}<CandidateApplicationForm roleId={role.roleId} roleCountry={role.roleCountry} /></main>;
}
