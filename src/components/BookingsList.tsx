...
import { isBeforeTargetHiringDate, roleAvailabilityRules, ruleToSlots, slotKey, virtualSlotsForRole, type InterviewAvailabilityRule } from "@/lib/interview-availability-rules";

type BookingKind = "voice" | "final";
type InterviewType = "AI Voice Interview" | "Final Interview";
type BookingRole = Pick<RoleRequestSummary, "roleId" | "jobTitle" | "targetHiringDate" | "hodEmail" | "hodAvailabilitySlots" | "voiceInterviewAvailabilityMode" | "voiceInterviewSlots" | "voiceInterviewAutoStartDate" | "voiceInterviewAutoEndDate" | "voiceInterviewTimezone" | "interviewAvailabilityRules"> & { finalBusyWindows?: string; hasActiveVoiceBookingLink?: boolean; hasActiveFinalBookingLink?: boolean };
...
function summarizeBookings(bookings: InterviewBooking[]) { const summary = { available: 0, booked: 0, blocked: 0, expired: 0, cancelled: 0, noShow: 0 }; bookings.forEach((booking) => { const normalized = booking.status.toLowerCase().replace(/\s+/g, ""); const status = (normalized === "noshow" ? "noShow" : normalized) as keyof typeof summary; if (status in summary) summary[status] += 1; }); return summary; }
function calendarDisplayBookings(bookings: InterviewBooking[]) {
  return bookings.filter((booking) => {
    const status = booking.status.toLowerCase().replace(/\s+/g, " ");
    if (hasStarted(booking)) return status === "booked" || status === "no show";
    return ["available", "booked", "blocked", "no show"].includes(status);
  });
}
    ...
    <div className="calendar-panel-heading">
      <div>
        ...
        <div className="calendar-panel-title">
          ...
          <InfoTip label={`About the ${title} calendar`}>HR manages availability rules. Select a date to see available times and booking status.</InfoTip>
        </div>
...
export default function BookingsList({ bookings: initialBookings, roles }: { bookings: InterviewBooking[]; roles: BookingRole[] }) {
  ...
  const roleOptions = useMemo(() => roles
    ...
    .map((role) => calendarBusyWindows[role.roleId] ? { ...role, finalBusyWindows: JSON.stringify(calendarBusyWindows[role.roleId]) } : role)
  ...
  const allBookings = useMemo(() => {
    const legacy = bookings
      .map((booking) => {
        ...
        if (blocked) return { ...booking, status: "Blocked", calendarEventStatus: "Conflict", calendarEventError: "HOD Google Calendar conflict" };
  ...
  }, [bookings, roleOptions, ruleOverrides]);
...
function hasFutureTime(booking: InterviewBooking) { try { return scheduledInstant(booking.date, booking.startTime || "00:00", booking.timezone || "Asia/Singapore").getTime() > Date.now(); } catch { return false; } }
