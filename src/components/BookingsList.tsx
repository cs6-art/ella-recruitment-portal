...
function CalendarPanel({ kind, month, bookings, selectedDate, selectedKind, onSelectDate }: { kind: "voice" | "final"; month: Date; bookings: InterviewBooking[]; selectedDate: string; selectedKind: "voice" | "final" | ""; onSelectDate: (date: string, kind: "voice" | "final") => void }) {
...
function NoShowAction({ booking }: { booking: InterviewBooking }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setMessage("Marked No Show");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to update status."); }
    finally { setSaving(false); }
  }
  return <div className="booking-no-show-action"><button type="button" className="booking-inline-action" disabled={saving} onClick={() => void submit()}>{saving ? "Saving..." : "Mark No Show"}</button>{message && <small role="status">{message}</small>}</div>;
}

export default function BookingsList({ bookings: initialBookings }: { bookings: InterviewBooking[] }) {
  ...
  const roleOptions = useMemo(() => [...new Set(bookings.map((booking) => booking.roleId).filter(Boolean))].sort(), [bookings]);
  ...
  const voiceBookings = filtered.filter((booking) => booking.interviewType.toLowerCase().includes("voice"));
  const finalBookings = filtered.filter((booking) => booking.interviewType.toLowerCase().includes("final"));
  const selectedBookings = selectedDate && selectedKind ? filtered.filter((booking) => dateKey(booking.date) === selectedDate && (selectedKind === "voice" ? booking.interviewType.toLowerCase().includes("voice") : booking.interviewType.toLowerCase().includes("final"))) : [];
  ...
  return <main className="container page bookings-page">
    ...
    {message && <div className="booking-page-message" role="status">{message}</div>}
    {showAvailability && <section className="card availability-card"><div><span className="eyebrow-dark">HR SCHEDULING</span><h2>Add Interview Availability</h2><p>Create a bookable time for either interview calendar and assign it to a role.</p></div><form className="availability-form" onSubmit={(event) => void addAvailability(event)}><label>Interview Type<select value={form.interviewType} onChange={(event) => updateForm("interviewType", event.target.value as FormState["interviewType"])}><option>AI Voice Interview</option><option>Final Interview</option></select></label><label>Role ID<input list="booking-role-options" required placeholder="e.g. ACC-001" value={form.roleId} onChange={(event) => updateForm("roleId", event.target.value)} /><datalist id="booking-role-options">{roleOptions.map((role) => <option value={role} key={role} />)}</datalist></label><label>Date<input type="date" required value={form.date} onChange={(event) => updateForm("date", event.target.value)} /></label><label>Start Time<input type="time" required value={form.startTime} onChange={(event) => updateForm("startTime", event.target.value)} /></label><label>End Time<input type="time" required value={form.endTime} onChange={(event) => updateForm("endTime", event.target.value)} /></label><label>Timezone<select value={form.timezone} onChange={(event) => updateForm("timezone", event.target.value)}><option>Asia/Singapore</option><option>Asia/Manila</option><option>Asia/Hong_Kong</option><option>UTC</option><option>America/Los_Angeles</option></select></label><div className="availability-form-actions"><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Adding..." : "Add to Calendar"}</button>{error && <span className="booking-form-error" role="alert">{error}</span>}</div></form></section>}
    ...
    <section className="calendar-navigation"><div><span>MONTHLY VIEW</span><strong>{monthLabel(month)}</strong></div><div><button type="button" className="btn btn-secondary" onClick={() => changeMonth(new Date())}>Today</button><button type="button" className="calendar-arrow" aria-label="Previous month" onClick={() => changeMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}>‹</button><button type="button" className="calendar-arrow" aria-label="Next month" onClick={() => changeMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}>›</button></div></section>
    <div className="booking-calendar-columns"><CalendarPanel kind="voice" month={month} bookings={voiceBookings} selectedDate={selectedDate} selectedKind={selectedKind} onSelectDate={selectDate} /><CalendarPanel kind="final" month={month} bookings={finalBookings} selectedDate={selectedDate} selectedKind={selectedKind} onSelectDate={selectDate} /></div>
    ...
    {selectedDate && selectedKind && <BookingDetails date={selectedDate} kind={selectedKind} bookings={selectedBookings} onClose={() => { setSelectedDate(""); setSelectedKind(""); }} />}
