...
import { isBeforeTargetHiringDate, isCurrentCalendarMonth, isStandardFinalInterviewSlot, isStandardVoiceInterviewSlot, parseAvailabilityRules, roleAvailabilityRules, ruleToSlots, slotKey, virtualSlotsForRole, type InterviewAvailabilityRule } from "@/lib/interview-availability-rules";
...
function virtualBookings(role: BookingRole, rulesOverride?: string): InterviewBooking[] {
  ...
  return (["AI Voice Interview", "Final Interview"] as InterviewType[]).flatMap((interviewType) => virtualSlotsForRole(source, interviewType).map((slot) => {
    if (interviewType === "Final Interview" && role.finalCalendarConnected !== true) return [];
...
function activeWindowCount(roles: BookingRole[], overrides: Record<string, string>) {
  const windows = new Set<string>();
  roles.forEach((role) => {
    const rules = roleAvailabilityRules({ ...role, interviewAvailabilityRules: overrides[role.roleId] ?? role.interviewAvailabilityRules });
