...
const initialRuleForm: RuleForm = { interviewType: "Final Interview", roleId: "", mode: "recurring", startDate: "", endDate: "", weekdays: [1, 2, 3, 4, 5], startTime: "09:00", endTime: "17:00", duration: "60", timezone: "Asia/Singapore", specificDate: "" };
...
function AvailabilityDrawer({ roles, form, setForm, saving, error, onClose, onSave }: { roles: BookingRole[]; form: RuleForm; setForm: React.Dispatch<React.SetStateAction<RuleForm>>; saving: boolean; error: string; onClose: () => void; onSave: () => void }) {
  ...
  return <div className="booking-drawer-backdrop" role="presentation" onClick={onClose}><aside className="booking-drawer" role="dialog" aria-modal="true" aria-labelledby="availability-drawer-title" onClick={(event) => event.stopPropagation()}><div className="booking-drawer-header"><div><span className="eyebrow-dark">HR SCHEDULING</span><h2 id="availability-drawer-title">Final interview availability</h2><p>Final interview times are read from the connected HR Google Calendar.</p></div><button type="button" className="booking-modal-close" aria-label="Close final interview availability" onClick={onClose}><UiIcon name="close" size={19} /></button></div><div className="booking-drawer-body"><label>Approved role<select required value={form.roleId} onChange={(event) => setForm((current) => ({ ...current, roleId: event.target.value }))}><option value="">Select an approved role</option>{roles.map((role) => <option key={role.roleId} value={role.roleId}>{role.jobTitle ? `${role.jobTitle} (${role.roleId})` : role.roleId}</option>)}</select></label>{selectedRole && <div className="availability-preview"><strong>Google Calendar source</strong><span>{selectedRole.finalCalendarConnected === true ? "Connected. Free times are available automatically." : "Connection not verified yet. Refresh after HR connects the calendar."}</span></div>}{error && <ActionFeedback kind="error">{error}</ActionFeedback>}</div><div className="booking-drawer-footer"><button type="button" className="btn btn-secondary" onClick={onClose} disabled={saving}>Close</button><button type="button" className="btn btn-primary" disabled={saving || !selectedRole} onClick={onSave}>{saving ? "Refreshing..." : "Refresh availability"}</button></div></aside></div>;
}
