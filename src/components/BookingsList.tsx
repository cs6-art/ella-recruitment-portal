...
function titleFor(kind: BookingKind) { return kind === "voice" ? "AI Voice Interview" : "HR Interview"; }
function displayInterviewType(value: string) { return value.toLowerCase().includes("final") ? "HR Interview" : value; }
...
function calendarHeading(kind: BookingKind) { return kind === "voice" ? "AI Voice Interview Calendar" : "HR Interview Calendar"; }
...
export default function BookingsList({ bookings: initialBookings, roles }: { bookings: InterviewBooking[]; roles: BookingRole[] }) {
  ...
  async function saveAvailability() { if (!selectedRole) return; setSaving(true); setError(""); setMessage(""); try { const response = await fetch("/api/bookings/calendar-busy", { credentials: "same-origin", cache: "no-store" }); const data = await response.json() as { success?: boolean; busyWindows?: Record<string, Array<{ start: string; end: string }>>; calendarConnected?: Record<string, boolean> }; if (!response.ok || data.success !== true) throw new Error("Unable to refresh HR calendar availability."); setCalendarBusyWindows(data.busyWindows || {}); setCalendarConnected(data.calendarConnected || {}); setShowAvailability(false); setForm(initialRuleForm); setMessage("HR interview availability refreshed from the HR Google Calendar."); router.refresh(); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to refresh HR calendar availability."); } finally { setSaving(false); } }
