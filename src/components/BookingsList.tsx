...
function CalendarPanel({ kind, month, bookings, selectedDate, selectedKind, onSelectDate }: { kind: "voice" | "final"; month: Date; bookings: InterviewBooking[]; selectedDate: string; selectedKind: "voice" | "final" | ""; onSelectDate: (date: string, kind: "voice" | "final") => void }) {
  ...
  return <section className={`card booking-calendar-card booking-calendar-${kind}`}><div className="calendar-panel-heading"><div><span className="calendar-panel-kicker">{kind === "voice" ? "VOICE SCREENING" : "HR INTERVIEW"}</span><div className="calendar-panel-title"><h2>{title} Calendar</h2><InfoTip label={`About the ${title} calendar`}>{kind === "voice" ? "Candidates use these available times to book their AI voice screening." : "Candidates use these available times to book their next interview with HR."}</InfoTip></div><p>{subtitle}</p><span className="calendar-availability-note"><span aria-hidden="true" /> Dates with open slots show <strong>Available</strong>.</span></div><div className="calendar-panel-counts"><span><UiIcon name="clock" size={13} /><b>{available}</b> Available</span><span><UiIcon name="check" size={13} /><b>{booked}</b> Booked</span></div></div><div className="booking-calendar-weekdays">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <span key={day}>{day}</span>)}</div><div className="booking-calendar-grid">{days.map((day, index) => { const key = day ? new Intl.DateTimeFormat("en-CA").format(day) : `empty-${index}`; const events = day ? byDate.get(key) || [] : []; const hasAvailable = events.some((booking) => booking.status.toLowerCase() === "available"); const today = day && new Intl.DateTimeFormat("en-CA").format(day) === new Intl.DateTimeFormat("en-CA").format(new Date()); const isSelected = selectedKind === kind && selectedDate === key; return <div className={`booking-calendar-day ${!day ? "is-empty" : ""} ${today ? "is-today" : ""} ${isSelected ? "is-selected" : ""}`} key={key}>{day && <button type="button" className="calendar-day-button" aria-label={`View ${title} events for ${dateLabel(key)}`} aria-pressed={isSelected} onClick={() => onSelectDate(key, kind)}><strong className="calendar-day-number">{day.getDate()}</strong><div className="calendar-day-events">{hasAvailable && <span className="calendar-day-availability">Available</span>}{events.slice(0, 3).map((booking) => <div className={`calendar-event ${booking.status.toLowerCase() === "booked" ? "is-booked" : "is-available"}`} key={booking.slotId}><b>{booking.startTime}</b><span>{booking.roleId || "Role"}</span><small>{booking.candidateName || "Available slot"}</small></div>)}{events.length > 3 && <small className="calendar-more">+{events.length - 3} more</small>}</div></button>}</div>; })}</div></section>;
}

function BookingDetails({ date, kind, bookings, onClose, onUpdated }: { date: string; kind: "voice" | "final"; bookings: InterviewBooking[]; onClose: () => void; onUpdated: (slotId: string) => void }) {
  const booked = bookings.filter((booking) => booking.status.toLowerCase() === "booked").length;
  const available = bookings.filter((booking) => booking.status.toLowerCase() === "available").length;
  const title = kind === "voice" ? "AI Voice Interview" : "Final Interview";
...
export default function BookingsList({ bookings: initialBookings, roles }: { bookings: InterviewBooking[]; roles: Pick<RoleRequestSummary, "roleId" | "jobTitle">[] }) {
  ...
  return <main className="container page bookings-page">
    ...
    {selectedDate && selectedKind && <BookingDetails date={selectedDate} kind={selectedKind} bookings={selectedBookings} onClose={() => { setSelectedDate(""); setSelectedKind(""); }} onUpdated={(slotId) => { setBookings((current) => current.map((booking) => booking.slotId === slotId ? { ...booking, status: "No Show" } : booking)); setSelectedDate(""); setSelectedKind(""); setMessage("Interview marked No Show."); }} />}
