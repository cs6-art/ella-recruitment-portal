...
import { canDeleteRoleRequest, canEditHodAvailability, canEditRoleRequest } from "@/lib/access-control";
...
export default function RoleDetails({ roleId, userEmail, canReviewRole, canApproveRole }: Props) {
  ...
  async function deleteRole() {
    const activeWarning = role && ["Approved", "Recruitment Setup", "Job Posted"].includes(role.status.trim())
      ...
      : "";
    if (!role || !window.confirm(`Delete role request ${role.roleId}? This cannot be undone.${activeWarning}`)) return;
  ...
  return <main className="container page role-details-page">
    <header className="role-sticky-header"><Link href="/roles" className="role-back-link"><span aria-hidden="true">←</span> Back to Role Requests</Link><div className="role-main-row"><div className="role-sticky-title"><h1>{role.jobTitle}</h1><p>{role.roleId}</p></div><div className="role-sticky-actions"><div className="role-sticky-actions-row"><span className={statusClass(role.status)}>{role.status}</span><button className="btn btn-secondary" type="button" disabled={refreshing} onClick={() => { setRefreshing(true); void loadRole(true).finally(() => setRefreshing(false)); }}>{refreshing ? "Refreshing…" : "↻ Refresh"}</button></div>{(canEdit || canDelete) && <div className="hero-actions role-request-actions"><Link className="btn btn-secondary" href={`/roles/${encodeURIComponent(role.roleId)}/edit`}>Edit request</Link>{canDelete && <button className="btn btn-secondary" type="button" disabled={deleting} onClick={() => void deleteRole()}>{deleting ? "Deleting..." : "Delete request"}</button>}</div>}</div></div><nav className="role-anchor-nav" aria-label="Role detail sections"><a href="#overview">Overview</a><a href="#details">Details</a><a href="#recruitment-setup">Recruitment Setup</a><Link href={`/roles/${encodeURIComponent(role.roleId)}/applicants`}>Applicants</Link><a href="#status-history">Status History</a></nav></header>
    ...
    <div className="role-layout"><div className="role-main-column">
      ...
      <Card title="HOD Availability and Screening"><DefinitionList><Field label="HOD / Interviewer Email" value={role.hodEmail} wide/><Field label="Availability Windows" value={availabilityValue(role.hodAvailabilitySlots, role.hodAvailabilityDates, role.hodAvailabilityTimes)} wide/><Field label="Custom HOD Screening Question 1" value={role.customScreeningQuestion1} wide/><Field label="Custom HOD Screening Question 2" value={role.customScreeningQuestion2} wide/><Field label="AI-Generated Screening Questions" value={role.aiGeneratedScreeningQuestions} wide/><Field label="Booking Link" value="Generated from interview slots after the workflow is connected." wide/></DefinitionList></Card>
    ...
    {["Approved", "Recruitment Setup", "Job Posted"].includes(role.status) && <HodAvailabilityEditor roleId={role.roleId} status={role.status} availability={role.hodAvailabilitySlots} editable={canEditHodAvailability({ email: userEmail, canReviewRole }, role)} onSaved={() => { void loadRole(true).then(() => router.refresh()); }} />}
    <RecruitmentSetupEditor roleId={role.roleId} status={role.status} editable={canReviewRole} updatedAt={role.recruitmentSetupUpdatedAt} updatedBy={role.recruitmentSetupUpdatedByName} updatedByEmail={role.recruitmentSetupUpdatedByEmail} onSaved={(expectedStatus) => { void loadRole(false, expectedStatus || "").then(() => router.refresh()); }} setup={setup}/>
