...
type RoleRequestDetails = {
  ...
  submittedByEmail: string; submittedByName: string; requesterEmail: string; requesterName: string; requesterType: string;
...
export default function RoleDetails({ roleId, canReviewRole, canApproveRole }: Props) {
  ...
  async function loadRole(preserveView = false) { if (!preserveView) setLoading(true); setError(""); try { const response = await fetch(`/api/roles/${encodeURIComponent(roleId)}`, { cache: "no-store", credentials: "same-origin" }); const raw = await response.text(); const data: ApiResponse = raw ? JSON.parse(raw) : {}; if (!response.ok || data.success !== true || !data.role) throw new Error(data.error || "Unable to load the role request."); setRole(data.role); setHistory(Array.isArray(data.history) ? data.history : []); } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to load the role request."); } finally { if (!preserveView) setLoading(false); } }
  ...
  return <main className="container page role-details-page">
    ...
    {successMessage && <div className="success-box" role="status">{successMessage}</div>}{warningMessage && <div className="warning-box" role="status">{warningMessage}</div>}
    ...
    <HrReview roleId={role.roleId} status={role.status} canReviewRole={canReviewRole} canApproveRole={canApproveRole} history={history} onSuccess={(message, warning) => { setSuccessMessage(message); setWarningMessage(warning || ""); void loadRole(); }} onConflict={() => void loadRole()}/>
