...
export default function RoleDetails({ roleId, userEmail, canReviewRole, canApproveRole }: Props) {
  ...
  return <main className="container page role-details-page">
    ...
    <RecruitmentSetupEditor roleId={role.roleId} status={role.status} editable={canReviewRole} updatedAt={role.recruitmentSetupUpdatedAt} updatedBy={role.recruitmentSetupUpdatedByName} updatedByEmail={role.recruitmentSetupUpdatedByEmail} onSaved={(expectedStatus) => { void loadRole(true, expectedStatus || "").then(() => router.refresh()); }} setup={setup}/>
