...
export default function ApplicantEditForm({ applicant }: { applicant: ApplicantEditValues }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (candidateName.trim().length < 2) issues.push({ field: "candidateName", label: "Full name", message: "Enter at least 2 characters.", href: "#applicant-edit-name" });
    ...
    if (!localNumber.trim()) issues.push({ field: "localNumber", label: "Preferred mobile number", message: "Enter a local mobile number.", href: "#applicant-edit-mobile" });
    if (issues.length > 0) {
