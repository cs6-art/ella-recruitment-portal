...
test("invite links can use a separate candidate page origin without breaking portal API CORS", () => {
  ...
  const candidatePage = read("public/application/index.html");
  ...
  assert.match(envExample, /RESUME_SCREENING_INVITE_BASE_URL=https:\/\/your-portal-domain\/application\/index\.html/);
  ...
  assert.match(candidatePage, /const PORTAL_API_BASE_URL = window\.location\.origin/);
